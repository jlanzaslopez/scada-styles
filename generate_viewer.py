...
function pill(letter, on) {
  return '<span class="pill ' + (on ? 'on' : '') + '">' + letter + '</span>';
}
...
function renderPreview(s, unused) {
  ...
  if (hasFill && s.fill.color) {
    if (s.fill.color.type === 'pattern') {
      bgStyle = 'background: ' + colorToCSS(s.fill.color) + ';';
    } else {
      bgStyle = 'background: ' + s.fill.color.css + ';';
    }
  }

  let borderStyle = '';
  if (hasOutline && s.outline.color) {
    borderStyle = 'border: ' + s.outline.weight + 'px solid ' + colorToCSS(s.outline.color) + ';';
  } else if (hasLine && s.line.color) {
    const c = s.line.color.type === 'pattern' ? s.line.color.fore : s.line.color.css;
    borderStyle = 'border: ' + s.line.weight + 'px solid ' + c + ';';
  }
  ...
  const blinkActive = (hasText && s.text.blink) || (hasFill && s.fill.blink) || (hasLine && s.line.blink) || (hasOutline && s.outline.blink);
  const blinkDot = blinkActive ? '<div class="blink-indicator" title="Blink activo"></div>' : '';

  return '<div class="preview" style="' + bgStyle + ' ' + borderStyle + '">' + textHTML + blinkDot + '</div>';
}

function renderMeta(s) {
  const lines = [];
  if (s.text.forced && s.text.font) {
    lines.push('<div class="meta-row"><span class="meta-label">FONT</span>' + s.text.font + ' ' + s.text.size + 'pt' + (s.text.bold ? ' bold' : '') + '</div>');
  }
  if (s.text.forced && s.text.color) {
    lines.push('<div class="meta-row"><span class="meta-label">COL</span>' + s.text.color.css + '</div>');
  }
  if (s.fill.forced && s.fill.color) {
    const fc = s.fill.color.type === 'pattern' ? 'pattern ' + s.fill.color.hatch : s.fill.color.css;
    lines.push('<div class="meta-row"><span class="meta-label">FILL</span>' + fc + '</div>');
  }
  if (s.line.forced) {
    const lc = s.line.color ? (s.line.color.type === 'pattern' ? 'pattern' : s.line.color.css) : '';
    lines.push('<div class="meta-row"><span class="meta-label">LINE</span>' + s.line.weight + 'px ' + s.line.pattern + ' ' + lc + '</div>');
  }
  if (s.outline.forced) {
    const oc = s.outline.color ? (s.outline.color.type === 'pattern' ? 'pattern' : s.outline.color.css) : '';
    lines.push('<div class="meta-row"><span class="meta-label">OUT</span>' + s.outline.weight + 'px ' + oc + '</div>');
  }
